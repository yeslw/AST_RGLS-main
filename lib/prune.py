...
from .local_swap import local_swap_one_linear, build_keep_mask_from_scores
...
def prune_ria(args, model, tokenizer, device=torch.device("cuda:0"), prune_n=0, prune_m=0):
    ...
    # RGLS: Check if enabled and warn for N:M sparsity
    rgls_enabled = getattr(args, 'refill_method', 'none') == 'swap' and getattr(args, 'swap_ratio', 0.0) > 0
    if rgls_enabled and prune_n != 0:
        print("[RGLS Warning] Local Swap does not support N:M sparsity. Disabling RGLS for this run.")
        rgls_enabled = False
    if rgls_enabled:
        print(f"[RGLS] Enabled with swap_ratio={args.swap_ratio}, bandwidth={args.swap_candidate_bandwidth}, alpha={args.swap_alpha}")
    ...
    for i in range(len(layers)):
        ...
        for name in subset:
            ...
            if args.prune_method == "wanda":
                ...
                # ============================================================
                # RIA Metric with Adaptive Structural Transition (AST)
                # Formula: Score = (beta * R_row + (1-beta) * R_col) * ||X||^alpha
                # 
                # AST Mode: beta(s) = 1 / (1 + exp(-k * (s - tau)))
                #   - tau = 0.4 (transition threshold)
                #   - k = 14 (sensitivity coefficient)
                # ============================================================
                
                # 0. Determine beta value (Adaptive or Fixed)
                if getattr(args, 'adaptive', False):
                    # Adaptive Structural Transition (AST)
                    # Sigmoid-based phase transition function
                    tau = 0.4   # Transition threshold
                    k = 14      # Sensitivity coefficient
                    s = args.sparsity_ratio
                    current_beta = 1.0 / (1.0 + np.exp(-k * (s - tau)))
                    # Print AST info only once per layer (first sublayer)
                    if name == list(subset.keys())[0]:
                        print(f"[AST Info] Sparsity: {s:.2f} -> Adaptive Beta: {current_beta:.4f}")
                else:
                    # Fixed beta from args
                    current_beta = args.beta
                
                # 1. Prepare data
                W_abs = torch.abs(W)
                # scaler_row shape: [in_features] -> [1, in_features] for broadcasting
                scaler = wrapped_layers[name].scaler_row.view(1, -1)
                
                # 2. Compute denominators (add epsilon to prevent division by zero)
                # Row sum: sum along dim=1, compresses columns, shape [out, 1]
                row_sum = torch.sum(W_abs, dim=1, keepdim=True) + 1e-8
                # Col sum: sum along dim=0, compresses rows, shape [1, in]
                col_sum = torch.sum(W_abs, dim=0, keepdim=True) + 1e-8
                
                # 3. Compute Relative Importance
                # R_row: weight proportion within its row (reflects output channel importance)
                R_row = W_abs / row_sum
                # R_col: weight proportion within its column (reflects input channel importance)
                R_col = W_abs / col_sum
                
                # 4. Weighted fusion with activation
                # Original RIA: (R_row + R_col) * (sqrt(X))^alpha
                # Parameterized: (beta * R_row + (1-beta) * R_col) * (sqrt(X))^alpha
                # When beta=0.5: 0.5*(R_row + R_col) * (sqrt(X))^alpha, ranking identical to original
                act_term = torch.pow(torch.sqrt(scaler), args.a)
                W_metric = (current_beta * R_row + (1 - current_beta) * R_col) * act_term
            W_mask = (torch.zeros_like(W_metric) == 1)  ## initialize a mask to be all False
            if prune_n != 0:
                ...
                # ============================================================
                # Unstructured Pruning with optional RGLS
                # ============================================================
                ...
                # Convert to keep_mask semantics: True = keep, False = prune
                keep_target = ~W_mask
                
                # RGLS: Residual-Guided Local Swap (if enabled)
                if rgls_enabled:
                    # Collect X_batches for residual computation
                    # Use the stored input activations from inps (already collected)
                    refill_batches = min(getattr(args, 'refill_calib_batches', 8), args.nsamples)
                    X_batches = []
                    
                    # We need to get input activations for this specific linear layer
                    # Run a few samples through and collect the inputs
                    layer_device = subset[name].weight.device
                    
                    for batch_idx in range(refill_batches):
                        # Get the input for this sample
                        inp_sample = inps[batch_idx]  # [seqlen, hidden_size]
                        
                        # For different layer types, we need different input handling
                        # The input to the linear layer depends on its position
                        # We'll use hook to capture actual inputs
                        captured_inp = []
                        def capture_hook(module, inp, out):
                            # inp[0] shape: [1, seqlen, hidden] or [seqlen, hidden]
                            x = inp[0].data
                            if len(x.shape) == 3:
                                x = x.reshape(-1, x.shape[-1])  # [tokens, in_features]
                            captured_inp.append(x.clone())
                        
                        hook = subset[name].register_forward_hook(capture_hook)
                        with torch.no_grad():
                            if "llama" in args.model:
                                _ = layer(inp_sample.unsqueeze(0), attention_mask=attention_mask, position_ids=position_ids)
                            elif "opt" in args.model:
                                _ = layer(inp_sample.unsqueeze(0), attention_mask=attention_mask)
                        hook.remove()
                        
                        if captured_inp:
                            X_batches.append(captured_inp[0].to(layer_device))
                    
                    if len(X_batches) > 0:
                        # Call RGLS local swap
                        keep_final, swap_stats = local_swap_one_linear(
                            W=W,
                            keep_target=keep_target,
                            ria_scores=W_metric,
                            X_batches=X_batches,
                            swap_ratio=getattr(args, 'swap_ratio', 0.0),
                            swap_candidate_bandwidth=getattr(args, 'swap_candidate_bandwidth', 0.005),
                            swap_alpha=getattr(args, 'swap_alpha', 0.5),
                            refill_out_chunk=getattr(args, 'refill_out_chunk', 512),
                            refill_clip_residual=getattr(args, 'refill_clip_residual', 0.0),
                            refill_dtype=getattr(args, 'refill_dtype', 'fp32'),
                            swap_debug=getattr(args, 'swap_debug', False),
                            layer_name=f"layer{i}.{name}"
                        )
                        
                        # Convert back to prune_mask
                        W_mask = ~keep_final
                        
                        # Clean up X_batches
                        del X_batches
                    else:
                        print(f"[RGLS Warning] No X_batches collected for layer{i}.{name}, skipping swap")
