...
def main():
    ...
    parser.add_argument("--beta", type=float, default=0.5, help="weight for row vs col relative importance. beta=0.5 reproduces original RIA")
    parser.add_argument("--adaptive", action="store_true", help="Enable Adaptive Structural Transition (AST): auto-compute beta based on sparsity using sigmoid function")
    
    # ============================================================
    # RGLS (Residual-Guided Local Swap) Parameters
    # ============================================================
    parser.add_argument("--refill_method", type=str, default="none", choices=["none", "swap"],
                        help="Refill method after initial pruning. 'none': no refill (baseline), 'swap': RGLS local swap")
    parser.add_argument("--swap_ratio", type=float, default=0.0,
                        help="Swap ratio as fraction of total params. swap_k = round(total * swap_ratio)")
    parser.add_argument("--swap_candidate_bandwidth", type=float, default=0.005,
                        help="Candidate pool bandwidth as fraction of total params")
    parser.add_argument("--swap_alpha", type=float, default=0.5,
                        help="Weight for combining benefit and RIA score: add_score = alpha*z(benefit) + (1-alpha)*z(ria)")
    parser.add_argument("--swap_debug", action="store_true",
                        help="Print detailed RGLS statistics for debugging")
    parser.add_argument("--refill_calib_batches", type=int, default=8,
                        help="Number of calibration batches for residual estimation")
    parser.add_argument("--refill_out_chunk", type=int, default=512,
                        help="Chunk size for out_features dimension (prevents OOM)")
    parser.add_argument("--refill_clip_residual", type=float, default=0.0,
                        help="If >0, clip residuals to [-v, v]")
    parser.add_argument("--refill_dtype", type=str, default="fp32", choices=["fp32", "fp16", "bf16"],
                        help="Dtype for accumulation matrix C")
